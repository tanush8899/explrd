...
const placeLabel = (p: SavedPlace) => p.city ?? p.name ?? p.formatted ?? p.place_id;
...
function topCountry(places: SavedPlace[]): { country: string; count: number } | null {
  ...
  for (const [country, count] of counts) if (!best || count > best.count) best = { country, count };
  return best;
}

// ─── Panel ────────────────────────────────────────────────────────────────────

type Props = { myPlaces: SavedPlace[]; myDisplayName: string };
...
export default function FriendsPanel({ myPlaces, myDisplayName }: Props) {
  ...
  const confirmRemove = (friend: FriendEntry) => {
    Alert.alert("Remove Friend", `Remove ${friend.displayName} from your friends?`, [
      ...
      { text: "Remove", style: "destructive", onPress: () => void removeFriend(friend.slug) },
    ]);
  };
    ...
    >
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.rail}>
        {friends.map((friend) => {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => {
                tap();
                selectFriend(isSelected ? null : friend.slug);
              }}
              onLongPress={() => confirmRemove(friend)}
            >
              <View style={[styles.avatarRing, isSelected && styles.avatarRingActive]}>
                <LinearGradient colors={gradientFor(friend.slug)} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.avatar}>
                  {isLoading ? (
        ...
        <TouchableOpacity
          style={styles.railItem}
          activeOpacity={0.8}
          onPress={() => { setAdding((v) => !v); setAddError(null); }}
        >
          <View style={[styles.avatarRing, adding && styles.avatarRingActive]}>
            <View style={styles.addCircle}>
              <Ionicons name="add" size={26} color={colors.inkSecondary} />
            </View>
          </View>
      ...
      {adding && (
        <Card style={styles.addBox}>
          ...
          <Text style={styles.addHint}>
            Paste their explrd profile link or username from Profile → public link.
          </Text>
          <View style={styles.addRow}>
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.8}
            >
              {addBusy ? <ActivityIndicator color="#ffffff" size="small" /> : <Text style={styles.addBtnText}>Add</Text>}
            </TouchableOpacity>
          </View>
      ...
      {friends.length === 0 && !adding && (
        <EmptyHint title="No friends yet" desc="Add a friend with their explrd profile link to explore their globe and compare your travels." />
      )}
      {friends.length > 0 && !selectedFriend && !adding && (
        <EmptyHint title="Pick a friend" desc="Tap a friend above to light up their places on the globe and see how your adventures compare." />
      )}

      {selectedFriend && !selectedData && loadingSlug === selectedFriend.slug && (
        <View style={styles.loadingBox}>
          <ActivityIndicator color={colors.blue} />
          <Text style={styles.loadingText}>Loading {firstName(selectedFriend.displayName)}'s globe…</Text>
        </View>
      )}
...
function FriendDetail({
  friendName, friendPlaces, friendStats, myName, myPlaces, myStats, mapFilter, setMapFilter,
}: {
  friendName: string; friendPlaces: SavedPlace[]; friendStats: ExplrdStats;
  myName: string; myPlaces: SavedPlace[]; myStats: ExplrdStats;
  mapFilter: FriendMapFilter; setMapFilter: (f: FriendMapFilter) => void;
}) {
  ...
  const fun = useMemo(() => {
    ...
    const myCountries = new Set(myPlaces.map((p) => (p.country ?? "").toLowerCase()).filter(Boolean));
    const sharedCountries = new Set(
      friendPlaces.map((p) => p.country).filter((c): c is string => !!c && myCountries.has(c.toLowerCase())),
    );
    return {
  ...
  const ahead = friendStats.percentWorldTraveled === myStats.percentWorldTraveled
    ? null
    : friendStats.percentWorldTraveled > myStats.percentWorldTraveled ? friendFirst : "You";

  return (
    <View>
      {/* Globe filter */}
      <Text style={styles.sectionLabel}>On the globe</Text>
      <SegmentedControl<FriendMapFilter>
        segments={[
          { key: "both", label: "Both of you" },
          { key: "friend", label: `Just ${friendFirst}` },
        ]}
        value={mapFilter}
      ...
      <View style={styles.legendRow}>
        ...
        {mapFilter === "both" && <LegendDot color="#8b5cf6" label="Both" />}
      </View>

      {/* VS hero */}
      <HeroCard eyebrow="World Explored" style={styles.vsCard}>
        <View style={styles.vsColumns}>
          ...
          <View style={styles.vsCol}>
            ...
            <Text style={[styles.vsPercent, { color: "#7cb1ff" }]}>{friendStats.percentWorldTraveled}%</Text>
          </View>
        </View>
      ...
      {/* Fun facts */}
      <Text style={[styles.sectionLabel, styles.sectionLabelSpaced]}>Fun facts</Text>

      {fun.lastPlace && (
        <FactCard icon="footsteps" color="#10b981" title={`${friendFirst}'s latest stamp`}
          big={placeLabel(fun.lastPlace)} sub={[fun.lastPlace.state, fun.lastPlace.country].filter(Boolean).join(", ")} />
      )}
      {fun.topCountry && (
        <FactCard icon="trophy" color="#f59e0b" title={`${friendFirst}'s most explored country`}
          big={fun.topCountry.country}
          sub={`${fun.topCountry.count} ${fun.topCountry.count === 1 ? "place" : "places"} stamped${fun.myTopCountry ? ` — yours is ${fun.myTopCountry.country}` : ""}`} />
      )}
      <FactCard icon="git-compare" color="#3b82f6" title="Been there together"
        big={fun.sharedPlaces.length === 0 ? "No overlap yet!" : `${fun.sharedPlaces.length} shared ${fun.sharedPlaces.length === 1 ? "city" : "cities"}`}
        sub={fun.sharedPlaces.length === 0
      ...
      {fun.sharedCountries.length > 0 && (
        <FactCard icon="earth" color="#06b6d4" title="Countries you've both touched"
      ...
      {fun.friendOnlyCount > 0 && (
        <FactCard icon="bulb" color="#8b5cf6" title="Trip inspiration"
          big={`${fun.friendOnlyCount} new ${fun.friendOnlyCount === 1 ? "idea" : "ideas"}`}
          sub={`${friendFirst} has been to ${fun.friendOnlyCount} ${fun.friendOnlyCount === 1 ? "place" : "places"} you haven't — ask them where to go next!`} />
      )}
    </View>
  );
}
...
function FactCard({ icon, color, title, big, sub }: {
  icon: React.ComponentProps<typeof Ionicons>["name"]; color: string; title: string; big: string; sub?: string;
}) {
  return (
...
const styles = StyleSheet.create({
  ...
  avatarRing: {
    width: 62, height: 62, borderRadius: 31, borderWidth: 2, borderColor: "transparent",
  ...
  addCircle: {
    width: 54, height: 54, borderRadius: 27, borderWidth: 1.5, borderStyle: "dashed",
    borderColor: colors.faint, alignItems: "center", justifyContent: "center", backgroundColor: colors.fill,
  },
  railName: { marginTop: 5, fontSize: 11, fontWeight: "500", color: colors.muted, maxWidth: 64 },
  railNameActive: { color: colors.ink, fontWeight: "700" },

  // Add form
  addBox: { marginBottom: 18 },
  addTitle: { fontSize: 16, fontWeight: "700", color: colors.ink, marginBottom: 4 },
  addHint: { fontSize: 13, color: colors.muted, lineHeight: 18, marginBottom: 12 },
  addRow: { flexDirection: "row", gap: 8 },
  addInput: {
    flex: 1, backgroundColor: colors.fill, borderRadius: 12,
    paddingHorizontal: 14, paddingVertical: 11, fontSize: 14, color: colors.ink,
  },
  addBtn: {
    backgroundColor: colors.ink, borderRadius: 12, paddingHorizontal: 18,
    alignItems: "center", justifyContent: "center", minWidth: 64,
  },
  addBtnText: { color: "#ffffff", fontSize: 14, fontWeight: "700" },
  addError: { marginTop: 10, fontSize: 13, color: colors.danger },

  // Empty / loading
  emptyBox: { marginTop: 8, alignItems: "center", paddingVertical: 26 },
  emptyTitle: { fontSize: 16, fontWeight: "700", color: colors.ink },
  emptyDesc: { fontSize: 14, color: colors.muted, marginTop: 6, textAlign: "center", lineHeight: 20, maxWidth: 280 },
  ...
  // Filter + legend
  filterControl: { marginBottom: 12 },
  legendRow: { flexDirection: "row", gap: 16, marginBottom: 18 },
  legendItem: { flexDirection: "row", alignItems: "center", gap: 5 },
  legendDot: { width: 8, height: 8, borderRadius: 4 },
  legendText: { fontSize: 12, color: colors.muted, fontWeight: "500" },

  // VS hero
  vsCard: { marginBottom: 4 },
  vsColumns: { flexDirection: "row", alignItems: "center", marginBottom: 16 },
  vsCol: { flex: 1, alignItems: "center" },
  vsName: { fontSize: 13, fontWeight: "600", color: "rgba(255,255,255,0.7)", marginBottom: 2 },
  vsPercent: { fontSize: 32, fontWeight: "800", color: "#ffffff", letterSpacing: -1 },
  vsVsText: { fontSize: 13, fontWeight: "800", color: "rgba(255,255,255,0.35)", paddingHorizontal: 10 },
  vsBarBlock: { marginBottom: 12 },
  vsBarLabels: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 5 },
  vsBarValue: { fontSize: 13, fontWeight: "700", color: "#ffffff", minWidth: 24 },
  vsBarLabel: { fontSize: 10, fontWeight: "600", color: "rgba(255,255,255,0.5)", letterSpacing: 0.5, textTransform: "uppercase" },
  ...
  vsFooter: { marginTop: 6, fontSize: 13, color: "rgba(255,255,255,0.78)", textAlign: "center" },

  // Fact cards
  factCard: { flexDirection: "row", alignItems: "flex-start", gap: 12, marginBottom: 10 },
  factBody: { flex: 1 },
